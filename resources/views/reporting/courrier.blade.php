        ...
        <div class="container-fluid">
            ...
            <div class="card">
              ...
              <div class="table-responsive"  >
                ...
                <div class="mb-3 row">
                    <label class="control-label text-end col-md-2">Expéditeur</label>
                    <div class="col-md-2">
                        <input type="text" id="expediteur_filter" class="form-control input-uppercase"
                    ...
                    <label class="control-label text-end col-md-2">Motif</label>
                    <div class="col-md-2">
                        <input type="text" id="motif_filter" class="form-control input-uppercase"
                        style="border-color: #ccc;"
                        onfocus="this.style.borderColor='#1e4db7'; this.style.boxShadow='0 0 0 0.2rem rgba(0, 123, 255, 0.25)';"
                        oninput="removeErrorStyles()"
                        onblur="this.style.borderColor='#ccc'; this.style.boxShadow='none';">
                    ...
                    <label class="control-label text-end col-md-2">Statut</label>
                    <div class="col-md-2">
                        
                        <select required name="statut_filter" id="statut_filter" class="form-control "  style="border-color: #ccc;"
                        onfocus="this.style.borderColor='#1e4db7'; this.style.boxShadow='0 0 0 0.2rem rgba(0, 123, 255, 0.25)';"
                        oninput="removeErrorStyles()"
                        onblur="this.style.borderColor='#ccc'; this.style.boxShadow='none';">
...
<style>
    ...
    overflow-x: auto;
                ...
                <table id="courrier_reporting" style="width: 100%;" class="table table-hover table-striped table-bordered  display  text-nowrap " >
                  <thead>
                    <tr>
                      <th>Id</th>
...
<script>
    $(document).ready(function() {
        ...
        var table_traite = $('#courrier_reporting').DataTable({
            serverSide: true,
            deferRender: true,
            dom: 'Bfrtip',
            scrollY: '300px',
            ...
            scrollCollapse: true,
            ajax: {
                url: "{{ route('reporting.courrier-table') }}", // L'URL de votre route Laravel
                type: 'GET',
            },
            processing: true,
            columns: [
                ...
                { data: 'affectation', name: 'affectation' },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        if (statut == 'ENREGISTREMENT'){
                        var htmlContent = `   <span class="mb-1 badge bg-warning text-dark" >ENREGISTREMENT</span> `;
                        }

                        if (statut == 'EN COURS DE TRAITEMENT'){
                        var htmlContent = `  <span class="mb-1 badge bg-success" >EN COURS DE TRAITEMENT</span> `;
                        }

                        if (statut == 'TRAITE'){
                        var htmlContent = `  <span class="mb-1 badge bg-primary" >TRAITE</span> `;
                        }
...
<script>
    ...
    var expediteur_filter = document.getElementById('expediteur_filter').value;
    var motif_filter = document.getElementById('motif_filter').value;
    var statut_filter = document.getElementById('statut_filter').value;
   
    $.ajax({
        type: 'GET',
        url: "{{ route('reporting.courrier-table-filter') }}", 
        data: {
            start_date: startDate,
            end_date: endDate,
            affectation_filter: affectationFilter,
            expediteur_filter: expediteur_filter,
        ...
        success: function(data) {
            ...
            var table = $('#courrier_reporting').DataTable();
            table.clear().destroy(); // Supprimer et détruire le DataTable existant
           
            
            var tables = $('#courrier_reporting').DataTable({
...
function resetDataTable() {
    ...
    document.getElementById('expediteur_filter').value = "";
    document.getElementById('motif_filter').value = "";
    document.getElementById('statut_filter').value = "";
...
<script>
  ...
  document.getElementById('u_statut').addEventListener('change', function() {
      ...
      if (statut === 'EN COURS DE TRAITEMENT') {
          ...
          dateFinTraitementInput.value = ''; // Assurez-vous que le champ de la date de fin est vide
